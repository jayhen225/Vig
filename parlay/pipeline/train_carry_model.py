...
sql_path = Path(__file__).parent / "sql" / "carry_share.sql"
...
carry_share = result["carry_share"].to_numpy()
...
print(f"Min carry share: {carry_share.min()}")
print(f"Max carry share: {carry_share.max()}")
print(f"Zeros: {(carry_share == 0).sum()}")
print(f"Ones: {(carry_share == 1).sum()}")

eps = 1e-6
carry_share = np.clip(carry_share, eps, 1 - eps)

with pm.Model() as model:
    # Level 1: League-wide average
    league_mu = pm.Normal("league_mu", mu=0.35, sigma=0.05)
    ...
    target = pm.Beta("carry", mu=pm.math.invlogit(player_mu[player_idx]), nu=kappa, observed=carry_share)
    # --- Sample from the posterior ---
    trace = pm.sample(1000, tune=1000, cores=1, random_seed=42)
    trace_path = Path(__file__).parent.parent / "data" / "traces" / "carry_share_trace"
